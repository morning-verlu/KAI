...
`--summary-out` appends Markdown, so it works with GitHub Step Summary files and local release-note artifacts without changing the `kaios.verify/v1` JSON printed to stdout. On failure, the summary starts with Verdict, Why It Failed, and Fix First sections so a maintainer can act before opening the JSON artifact. When `--baseline ... --check` finds behavior changes, it also includes a What Changed table with the first stable runtime differences.
