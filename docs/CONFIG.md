...
It emits `kaios.config-validation/v1` with `valid`, workflow metadata, agent ids, validation errors, human-readable `next` commands, and structured `nextActions` with stable action ids for automation.
