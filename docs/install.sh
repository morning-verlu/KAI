...
if command -v kaios >/dev/null 2>&1; then
  if [ -f README.md ]; then
    PROJECT_RUN="kaios run --index . --context README.md --out artifacts/project.md --force \"summarize this project\""
  else
    PROJECT_RUN="kaios run --index . --out artifacts/project.md --force \"summarize this project\""
  fi
  echo "Try:"
  echo "  kaios doctor"
  ...
  if [ -f README.md ]; then
    PROJECT_RUN="${BIN_DIR}/kaios run --index . --context README.md --out artifacts/project.md --force \"summarize this project\""
  else
    PROJECT_RUN="${BIN_DIR}/kaios run --index . --out artifacts/project.md --force \"summarize this project\""
  fi
