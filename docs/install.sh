...
if command -v kaios >/dev/null 2>&1; then
  ...
  echo "  kaios evidence latest --out artifacts/run.capsule.json --force"
else
  if [ -f README.md ]; then
  ...
  echo "  ${BIN_DIR}/kaios evidence latest --out artifacts/run.capsule.json --force"
fi
