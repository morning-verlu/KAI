...
class KaiosCli(
    ...
    private fun renderTourText(report: TourReport, out: PrintStream) {
        out.println("KAI OS Evidence OS tour")
        ...
        out.println()
        out.println("what happened:")
        out.println("  1. Created a disposable Git workspace.")
        out.println("  2. Ran no-key quickstart with the deterministic provider.")
        out.println("  3. Changed ${report.changedFile} and reviewed the diff.")
        out.println("  4. Wrote a Markdown review, process trace, replayable capsule, and recovery dry-run report.")
        out.println()
        out.println("product proof:")
        out.println("  Agent = Process   -> run 'kaios ps ${report.runId}'")
        out.println("  Tool  = Syscall   -> inspect '${displayPath(Paths.get(report.trace))}'")
        out.println("  Run   = Evidence  -> replay '${displayPath(Paths.get(report.capsule))}'")
        out.println("  CI    = Gate      -> compare a future baseline with 'kaios evidence --baseline ... --check'")
        out.println()
        ...
        out.println()
        out.println("commands executed:")
        report.commands.forEach { step -> out.println("  ${step.name}: exit=${step.exitCode} command=\"${step.command}\"") }
        out.println()
        out.println("proof pack:")
        out.println("  https://morning-verlu.github.io/KAI/proof-pack.html")
        out.println()
        out.println("next:")
